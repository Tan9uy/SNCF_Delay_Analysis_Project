...
if [[ ! -f "${TF_OUT_FILE}" ]]; then
  echo "ERROR: out file ${TF_OUT_FILE} doesn't exist" >&2
  usage
fi

MASTERS=$(jq -r '.master_ips.value | to_entries[]'  < ${TF_OUT_FILE})
WORKERS=$(jq -r '.worker_ips.value | to_entries[]'  < ${TF_OUT_FILE})
mapfile -t MASTER_NAMES < <(jq -r '.key'  <(echo "${MASTERS}"))
mapfile -t WORKER_NAMES < <(jq -r '.key'  <(echo "${WORKERS}"))

API_LB=$(jq -r '.control_plane_lb_ip_address.value' < ${TF_OUT_FILE})
